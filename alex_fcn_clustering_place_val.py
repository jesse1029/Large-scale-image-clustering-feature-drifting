...
fnv='/Data/jess/places2/val_large/'
fnv2='/Data/jess/places2/label/places365_val.txt'
...
def val_batch(bs, ss=-1):
	...
	for k in range(bs):
		...
		try:
			...
			fct=fct+1
		if (fct>0):
			print('Fuck, what the problem!! it is file corrupted problem...')
		...
		for v in range(vst,1000):
			...
			for k in range(0, vlen1, batch_size):
				...
				if step % display_step == 1:
					...
					print ("Iter=" + str(step*batch_size) + "/epcho=" + str(np.floor((step*batch_size)/vlen1)) + ", Loss= " + "{:.6f}".format(loss) + ", Training Accuracy=" + "{:.5f}".format(acc) + ", lr=" + str(lr) + ", Sec.=" + "{:.5f}".format(time.time()-tii))
