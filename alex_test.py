...
fn='/home/jess/Disk1/ilsvrc12/ILSVRC2012_img_val/'
fn2='/home/jess/Disk1/ilsvrc12/label/val.txt'
...
def imagenet_batch(bs):
	...
	for k in range(bs):
		...
		try:
			...
			data[cnt,:,:,:]=cropping(im, phase="TEST")
...
def trainer():
	...
	with tf.Session() as sess:
		...
		lr = 1e-3
		step = 1
		saver.restore(sess, "/home/jess/Disk1/ilsvrc12/tf_resnet_ccnn_model_iter180001.ckpt-180001")
		# Keep training until reach max iterations
		aacc=0.0

		while step * batch_size < glen1:

			batch_xs, batch_ys = data_queue.get()

			aacc = aacc + sess.run(accuracy, feed_dict={X: batch_xs, Y: batch_ys, keep_prob: 1.})
			if step % display_step == 1:
				print "Average Accuracy: " + str(aacc/(step)) + " for " +  str(step*batch_size) + " test images..."
				kk=sess.run(info2, feed_dict={X: batch_xs, keep_prob: 1.})
				#print str(kk)
			step += 1
